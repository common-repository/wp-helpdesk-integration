...
class SkinnyBaseDbController_HelpdeskIntegration extends PDO {
   ...
   */
   public static function getConnection($dbKey = '', $mode='r+') { // empty $dbKey must be '' and NOT null!!!
     ...
     if (!isset(self::$connections[$dbKey][$mode]) || empty(self::$connections[$dbKey][$mode])) {
       ...
       $dsn = $db_config["dbdriver"].":dbname=".$dbName.";host=".$db_config["dbhost"];
       try {
         return new SkinnyDbController_HelpdeskIntegration($dbKey, $mode, $dsn, $db_config["dbuser"], $db_config["dbpassword"]);
       } catch (PDOException $e) {
         throw new SkinnyDbException_HelpDeskIntegration($e->getMessage(), $e->getCode());
       }
     } else {
