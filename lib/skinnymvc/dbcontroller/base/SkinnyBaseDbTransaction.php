...
class SkinnyBaseDbTransaction_HelpDeskIntegration {
    ...
    public static function create($p=array()) {
      $t = new SkinnyDbTransaction_HelpDeskIntegration();
      if (!empty($p)) {
         $t->addManyORMs($p);
      }
      return $t;
    ...
    public function begin() {
       if(empty($this->currentDbKey)) {
          throw new SkinnyDbException_HelpDeskIntegration('No DB connection.');
       }
       ...
       $con = SkinnyDbController_HelpdeskIntegration::getWriteConnection($this->currentDbKey);
    ...
    public function addORM($ormObj) {
       if (!is_object($ormObj)) {
          throw new SkinnyDbException_HelpDeskIntegration('Non-object sent to addORM().');
       }
       ...
       if (is_null($this->currentDbKey)) {
          if (isset(self::$dbTransactions[$dbKey])) {
             throw new SkinnyDbException_HelpDeskIntegration('An open transaction already exists for this DB key.');
          }
          ...
          if($this->currentDbKey != $dbKey) {
             throw new SkinnyDbException_HelpDeskIntegration('DbKey mismatch.');
          }
       }

       if ($ormObj->isInAnyTransaction()) {
          throw new SkinnyDbException_HelpDeskIntegration('Object is already in a transaction');
       }
    ...
    public function commit() {
       ...
       if (!$this->transactionActive) {
          throw new SkinnyDbException_HelpDeskIntegration('Transaction not active');
       }
