...
class SkinnyBaseControllerHelpDeskIntegration {
   ...
   */
    public function main()
    {
           ...
           // Slash after the module missing?
            ...
            if (  $hasMissingSlash  ) {
                ...
                if (  $this->allowModulesAsFiles   ) {
                    ...
                    echo file_get_contents(WP_HELPDESK_INTEGRATION_DIRECTORY."templates/404.php");
                    exit;
                }
            }
        ...
        $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."lib/skinnymvc/core/base/*.php");
        $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."lib/skinnymvc/core/*.php");
		
        // Get the db controller classes
        $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."lib/skinnymvc/dbcontroller/base/*.php");
        $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."lib/skinnymvc/dbcontroller/*.php");
			
        // Get the controller classes
        $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."lib/skinnymvc/controller/base/*.php");
        $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."lib/skinnymvc/controller/*.php");
			
        //Get all Model classes
        if (SkinnySettingsForHelpDeskIntegration::$CONFIG['preload model']) {
            $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."lib/skinnymvc/model/*.php");
            $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."lib/skinnymvc/model/base/*.php");
        }

        //Initialize session
        if (SkinnySettingsForHelpDeskIntegration::$CONFIG['session persistency']) {
            $this->skinnyUser = SkinnyUser_HelpDeskIntegration::getUser();
        }

        //Get all plugins
        $this->require_once_many(WP_HELPDESK_INTEGRATION_DIRECTORY."plugins/skinnyPlugin*.php");
    ...
    {
        if (!file_exists(WP_HELPDESK_INTEGRATION_DIRECTORY."modules/$module/actions/actions.php")) {
            //Error: Action does not exist
            header("HTTP/1.1 404 Not Found");
            echo file_get_contents(WP_HELPDESK_INTEGRATION_DIRECTORY."templates/404.php");
            exit;
        }
        require_once(WP_HELPDESK_INTEGRATION_DIRECTORY."modules/$module/actions/actions.php");
        ...
        if ( $moduleObj->allowUndefinedActions()) {
            ...
            if (  is_callable(array($moduleObj, $actionMethod))  ) {
                ...
                if (!file_exists(WP_HELPDESK_INTEGRATION_DIRECTORY."modules/$module/templates/$action.php")) {
                    //Error: Action does not exist
                    header("HTTP/1.1 404 Not Found");
                    echo file_get_contents(WP_HELPDESK_INTEGRATION_DIRECTORY."templates/404.php");
                    exit;
            ...
            if (SkinnySettingsForHelpDeskIntegration::$CONFIG['debug']) {
                global $__DEBUG;
        ...
        if (!file_exists(WP_HELPDESK_INTEGRATION_DIRECTORY."modules/$module/templates/$action.php")) {
            //Error
            throw new SkinnyException_HelpDeskIntegration("Template for module $module, action $action does not exist.");
            exit;
        }

        $actionTemplateSource = file_get_contents(WP_HELPDESK_INTEGRATION_DIRECTORY."modules/$module/templates/$action.php");
        ...
        $rand = rand(0, 99);
        if ($rand == 1) {
            SkinnyUser_HelpDeskIntegration::cleanup();
        }
    }
    ...
    protected function checkAuthentication($moduleObj, $skinnyUser, $param) {
        ...
        if ($moduleObj->authenticatedOnly()) {
            if (!$skinnyUser->isAuthenticated()) {
                //Not authenticated!
                if (isset(SkinnySettingsForHelpDeskIntegration::$CONFIG['unauthenticated default module'])) {
                    if (isset(SkinnySettingsForHelpDeskIntegration::$CONFIG['unauthenticated default action'])) {
                        $moduleObj->redirect(SkinnySettingsForHelpDeskIntegration::$CONFIG['unauthenticated default module'], SkinnySettingsForHelpDeskIntegration::$CONFIG['unauthenticated default action'], $param);
                    } else {
                        $moduleObj->redirect(SkinnySettingsForHelpDeskIntegration::$CONFIG['unauthenticated default module'], "index", $param);
                    }
                } else {
   ...
   */
   ...
   private function processLayout(&$skinny_content, $layoutData, $skinnyUser, $module, $action, $layout) {
      include_once WP_HELPDESK_INTEGRATION_DIRECTORY.'templates/'. $layout .'.php';
   }
